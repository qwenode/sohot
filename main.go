...
func main() {
	...
	// 设置控制台窗口标题为: profile名称 - 项目文件夹名称 - SoHot
	if cwd, err := os.Getwd(); err == nil {
		projectName := cwd[strings.LastIndex(cwd, string(os.PathSeparator))+1:]
		boot.SetConsoleTitle(key + " - " + projectName + " - SoHot")
	} else {
		boot.SetConsoleTitle(key + " - SoHot")
	}
