...
func Running(input types.Run) {
	consume(stopRunning)
	cmd := exec.Command(boot.V.Build.Name, input.Command...)
	...
	go io.Copy(boot.StdoutWriter, stdoutPipe)
	go io.Copy(boot.StderrWriter, pipe)
	// 捕获 cmd 的本地副本，避免竞态条件
...
func Building(input types.Run) {
	if input.Only {
		for {
			...
			case <-change:
				...
				if _, err := os.Stat(boot.V.Build.Name); os.IsNotExist(err) {
					log.Warn().Str("file", boot.V.Build.Name).Msg("Executable file not found, delaying restart to wait for compilation")
					continue // Skip this restart, continue waiting
				}

				log.Info().Str("file", boot.V.Build.Name).Msg("Executable file exists, performing restart")
				isFirstRunLock.Lock()
				first := isFirstRun
				if isFirstRun {
					isFirstRun = false
	...
	commands = append(commands, boot.V.Build.Command...)
	commands = append(commands, "-o", boot.V.Build.Name, boot.V.Build.Package)
	var cmd *exec.Cmd
	...
	for {
		...
		case <-change:
			...
			// Set new delay timer
			delayTimer = time.AfterFunc(time.Duration(boot.V.Build.Delay)*time.Millisecond, func() {
				...
				tempExecName := boot.V.Build.Name + ".tmp_" + time.Now().Format("20060102150405")

				// Clean up previously existing temporary files
				matches, _ := filepath.Glob(boot.V.Build.Name + ".tmp_*")
				for _, tempFile := range matches {
					os.Remove(tempFile)
				}
				...
				go func(buildCmd *exec.Cmd, tempFile string) {
					...
					if _, err := os.Stat(boot.V.Build.Name); !os.IsNotExist(err) {
						if err := forceDeleteFile(boot.V.Build.Name); err != nil {
							log.Warn().Err(err).Str("file", boot.V.Build.Name).Msg("Failed to delete old executable file")
						}
					}

					// Rename temporary file to official file
					if err := os.Rename(tempFile, boot.V.Build.Name); err != nil {
						log.Err(err).Str("temp_file", tempFile).Str("target_file", boot.V.Build.Name).Msg("Failed to rename file")
						// Clean up temporary file
						os.Remove(tempFile)
						return
					}

					log.Info().Str("file", boot.V.Build.Name).Msg("Executable file updated successfully, starting new program")
			...
			startCountdown(boot.V.Build.Delay)
...
func Watching(input types.Run) {
	...
	if input.Only {
		watchDirs[filepath.Dir(boot.V.Build.Name)] = true
	} else {
		for _, s := range boot.V.Watch.Include {
			filepath.WalkDir(
...
func isExclude(path string) bool {
	...
	for _, s := range boot.V.Watch.Exclude {
